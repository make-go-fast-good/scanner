...
const process = function processPlcData(data, _num) {
  ...
  for (let i = 0; i < parseInt(_num) + 5; i++) {
    ...
    let row = dataKeys.filter((val) => {
      ...
      return (
        parseFloat(val) >= 20004 + i * 92 && parseFloat(val) <= i * 92 + 20092
      );
    });
    ...
    // Remove orientation char from barcode output
    if (
      plcData[i].barcode.length === 10 &&
      (plcData[i].barcode.slice(-1) == "0" ||
        plcData[i].barcode.slice(-1) == "1" ||
        plcData[i].barcode.slice(-1) == "2")
    ) {
      plcData[i].barcode = plcData[i].barcode.substring(
        0,
        plcData[i].barcode.length - 1
      );
    }
    ...
    if (plcData[i].data["anz_noread"] < 0)
      plcData[i].data["anz_noread"] += 65535;
  }
  return plcData;
};
